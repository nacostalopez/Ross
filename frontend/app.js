...
// generated by tools/agentes/). Decorative only, so a missing script just means
// no drawings.
// ---------------------------------------------------------------------------
...
const WIDGET_AGENT_HEAD = { connector_status: "conectores" };

function renderPanelWidgetShell(w) {
  const body = PANEL_WIDGET_BODY[w.type];
  const head = WIDGET_AGENT_HEAD[w.type]
    ? `<span class="pxa pxa-sprite" style="--w:12;--h:11" data-agent-head="${WIDGET_AGENT_HEAD[w.type]}" aria-hidden="true"></span>`
    : "";
  return `
    <div class="panel">
      <div class="widget-card-header">
        <div class="widget-title-row">${head}<h3>${WIDGET_LABELS[w.type]}</h3></div>
...
async function loadProfilePanel() {
  paintUserAgent(document.getElementById("profile-bust"), "agentBust");
